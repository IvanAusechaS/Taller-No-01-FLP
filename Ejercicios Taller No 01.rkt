...
; list-set
; BNF:
; <list-set> ::= (lambda (<L> <n> <x>) <cond-expr>)
; <L> ::= <list>
; <n> ::= <integer>
; <x> ::= <element>
; <cond-expr> ::= (cond
...
; swapper
; BNF:
; <swapper> ::= (lambda (<E1> <E2> <L>) <cond-expr>)
; <E1> ::= <element>
; <E2> ::= <element>
...
;(swapper 'dog 'cat '(dog cat dog cat dog))

; Ejercicio No 09
; inversions
; BNF:
; <inversions> ::= (lambda (<L>) <if-expr>)
; <L> ::= <list>
; <if-expr> ::= (if (null? <L>) 0 (count (car <L>) (cdr <L>) (inversions (cdr <L>))))
; <count> ::= (lambda (<x> <L> <acum>) <cond-expr>)
; <x> ::= <integer>
; <acum> ::= <integer>
; <cond-expr> ::= (cond
;                   [(null? <L>) <acum>]
...
; filter-acum
; BNF:
; <filter-acum> ::= (lambda (<a> <b> <F> <acum> <filter>) <if-expr>)
; <a> ::= <integer>
; <b> ::= <integer>
; <F> ::= <function>
; <acum> ::= <value>
...
;(filter-acum 5 15 + 0 even?)
;(filter-acum 1 7 * 1 odd?)


; count-odd-and-even
; BNF:
; <count-odd-and-even> ::= (lambda (<arbol>) <if-expr>)
; <arbol> ::= <tree>
; <if-expr> ::= (if (null? <arbol>) '(0 0)
...
;(count-odd-and-even '(7 () ()))
;(count-odd-and-even '(4 (2 (1 () ()) (3 () ())) (6 (5 () ()) (7 () ()))))

; Pascal
; BNF:
; <pascal> ::= (lambda (<N>) <cond-expr>)
; <N> ::= <integer>
; <cond-expr> ::= (cond
;                   [(= <N> 0) '(0)]
;                   [(= <N> 1) '(1)]
;                   [else (next-row (pascal (- <N> 1)))])
; <next-row> ::= (lambda (<L>) <loop>)
; <loop> ::= (let loop ([prev <L>] [result '(1)])
;               (if (null? (cdr <prev>))
